...
class EmployeeRepositoryCached:
    CHUNK_SIZE = 1000
    ...
    async def create(cls, session: AsyncSession):
        instance = cls()
        await instance._initialize_cache(session)
        return instance

    async def _initialize_cache(self, session: AsyncSession):
        self.cache.employees = {
            emp.id: emp for emp in await EmployeeRepositoryCached.get_all_employees(session)
        }
        self.cache.time_records = {
            (tr.employee_id, tr.subdivision_id, tr.work_date.strftime('%x')): tr
            for tr in await EmployeeRepositoryCached.get_all_time_records(session)
        }

    async def get_employee_by_id(self, employee_id: int):
        return self.cache.employees.get(employee_id, None)

    async def get_employee_by_pin(self, pin: str):
        result = [emp for emp in self.cache.employees if emp.pin == pin]
        return result[0] if result else None

    async def get_employee_by_tab_no(self, tab_no: str):
        result = [emp for emp in self.cache.employees if emp.tab_no == tab_no]
        return result[0] if result else None
