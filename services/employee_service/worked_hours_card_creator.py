...
svg_file_name = file_path = os.path.join(os.path.dirname(__file__), 'worked_hours.svg')
...
class WorkedHoursCardCreator:
    ...
    def __init__(self, locale='en'):
        if WorkedHoursCardCreator.tree is None:
            logger.warning('Reading svg disk file')
            WorkedHoursCardCreator.tree = etree.parse(svg_file_name)

        self.svg_tree = copy.deepcopy(WorkedHoursCardCreator.tree)
